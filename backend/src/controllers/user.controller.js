...
exports.getAllUsers = async (req, res) => {
    try {
        const users = await User.find().select("-password").populate("role", "roleName").populate({ // Student → ParentStudent → Parent → User
            path: "studentInfo",
            populate: [
                {
                    path: "parentStudent",
...
exports.getUserById = async (req, res) => {
    try {
        const user = await User.findById(req.params.id).populate("role", "roleName roleDescription").select("-password");
...
exports.createUser = async (req, res) => {
    try {
        ...
        const user = new User({username, password, email, role: roleId});

        await user.save();

        const userResponse = await User.findById(user._id).populate("role", "roleName").select("-password");
...
exports.updateUser = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, updateData, {new: true}).populate("role", "roleName").select("-password");
