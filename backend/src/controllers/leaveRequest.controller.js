...
exports.getLeaveRequests = async (req, res) => {
    try {
        ...
        const requests = await LeaveRequest.find(query).populate("student", "firstName lastName").populate("reviewedBy", "username").sort({requestedAt: -1});
...
exports.reviewLeaveRequest = async (req, res) => {
    try {
        ...
        res.json({success: true, message: `Leave request ${status} successfully`, data: leaveRequest});
    } catch (error) {
...
exports.deleteLeaveRequest = async (req, res) => {
    try {
        ...
        if (leaveRequest.status === "approved") {
            return res.status(400).json({success: false, message: "Cannot delete approved leave requests"});
        }
